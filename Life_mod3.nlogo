patches-own [
  living1?         ;; indicates if the cell is in state living1
  living2?         ;; indicates if the cell is in state living2
  living-forever?
  live-neighbors1  ;; counts how many neighboring cells are alive and in state1
  live-neighbors2  ;; counts how many neighboring cells are alive and in state2
  live-neighbors-forever
  live-neighbors
]

to setup-blank
  clear-all
  ask patches [ cell-death-init ]
  reset-ticks
end
...
to setup-random
  ...
  ask patches
    [
     (ifelse
      random-float 100.0 < initial-density
      [let choice random 3
       (ifelse
        choice = 0
        [ cell-birth1 ]
...
to cell-birth-forever
  set living1? false
  set living2? false
  set living-forever? true
  set pcolor red
end
...
to cell-death-running
  set living1? false
  set living2? false
  (ifelse living-forever? = true
  [set pcolor red]
  [set pcolor bgcolor])
end

to cell-death-forever
  set living1? false
  set living2? false
  set living-forever? false
  set pcolor bgcolor
...
to go
  ask patches
    [ set live-neighbors1 count neighbors with [living1?] ]
  ask patches
    [ set live-neighbors2 count neighbors with [living2?] ]
  ask patches
    [ set live-neighbors count neighbors with [living1? or living2?] ]
  ask patches
    [ set live-neighbors-forever count patches in-radius 3 with [living-forever?] ]
  ...
  ask patches
    [ (ifelse
      live-neighbors-forever >= 2 and living-forever? = true
      [cell-death-forever]
      live-neighbors = 3 and live-neighbors1 > live-neighbors2 and living-forever? = false
      [ cell-birth1 ]
      live-neighbors = 3 and live-neighbors2 > live-neighbors1 and living-forever? = false
      [ cell-birth2 ]
      live-neighbors = 2
      [ ]
      [ cell-death-running ]) ]
...
;;to draw-cells
;;  let erasing? [living1?] of patch mouse-xcor mouse-ycor
;;  while [mouse-down?]
...
"" ""
PENS
"default" 1.0 0 -11221820 true "" "plot count patches with [living1?]"
"pen-1" 1.0 0 -13840069 true "" "plot count patches with [living2?]"
"pen-2" 1.0 0 -2674135 true "" "plot count patches with [living-forever?]"
